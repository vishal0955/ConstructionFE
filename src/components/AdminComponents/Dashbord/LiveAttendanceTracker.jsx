...
const LiveAttendanceTracker = () => {
  ...
  // Filter data based on search criteria (site, role, and name)
  const filteredData = sampleData.filter((row) => {
    return (
      (!filters.site || row.site === filters.site) &&
      (!filters.role || row.role === filters.role) &&
      (!filters.name ||
        row.name.toLowerCase().includes(filters.name.toLowerCase())) // Case-insensitive search for name
    );
  });
  ...
  return (
    <div>
      <h5 className="mb-3">Live Site Attendance</h5>
