...
//     auditedBy: "", // Replace with actual logged-in user data
//     machineId: "",
//     odometerReading: "",
//     nextServiceDue: "",
//     plantType: "",
...
//         ? []
//         : predefinedChecklists[selectedPlantType] || [],
//     }));
//   }
...
//       setFormData((prev) => ({
//         ...prev,
//         checklist: [...prev.checklist, { item: customChecklistItem, status: "", comment: "", photo: null }],
//       }));
//       setCustomChecklistItem("");
//       setShowChecklistModal(false);
//     }
...
//                   <tr key={index}>
//                    <td> { console.log(typeof item === "string" ? item : "" )}
//                     {typeof item === "string" ? item : item.item}</td>
...
const AuditEquipment = () => {
  ...
  const [formData, setFormData] = useState({
    auditDate: new Date().toISOString().split("T")[0],
    auditedBy: "", // Replace with actual logged-in user data when available
    machineId: "",
    odometerReading: "",
    nextServiceDue: "",
    plantType: "",
  ...
  const handlePlantTypeChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      plantType: selectedPlantType,
      // If custom is selected, keep the checklist empty
      // Otherwise, use the predefined checklist or empty array if not found
      checklist: selectedPlantType === "Custom"
        ? []
  ...
  const handleChecklistStatusChange = (index, status) => {
    ...
    // Show task modal only when setting status to "Not OK"
    if (status === "Not OK") {
      // Set the current item in taskDetails
      setTaskDetails({
        ...taskDetails,
  ...
  const handleSubmit = (e) => {
    ...
    // Convert any string checklist items to object format
    ...
    // If it's a custom plant type, include the custom name as the actual plant type
  ...
  // Add button to add custom checklist item
