...
import { Search, Plus, X, Edit2, Save, AlertTriangle, Download, Eye } from 'lucide-react';
...
export default function PPERegisterUI() {
    ...
    highRisk: [
      ...
      },
      {
        ...
        ppeItems: [
          ...
          { name: 'Respirator', mandatory: true, standard: 'AS/NZS 1716' },
          { name: 'Cut-Resistant Gloves', mandatory: true, standard: 'AS/NZS 2161' },
          { name: 'Steel Cap Boots', mandatory: true, standard: 'AS/NZS 2210' },
          { name: 'Hearing Protection', mandatory: true, standard: 'AS/NZS 1270' },
          { name: 'High Visibility Vest', mandatory: true, standard: 'AS/NZS 4602' }
        ],
    ...
    generalTrades: [
      ...
      },
      {
        ...
        ppeItems: [
          ...
          { name: 'Fire-Resistant Clothing', mandatory: true, standard: 'AS/NZS 4824' },
          { name: 'Steel Cap Boots', mandatory: true, standard: 'AS/NZS 2210' },
          { name: 'Respirator', mandatory: true, standard: 'AS/NZS 1716' }
        ],
  ...
  const handleAddTemplate = () => {
    if (newTemplate.taskName && selectedCategory) {
      const updatedTemplates = { ...templates };
      const newId = Math.max(...Object.values(templates).flat().map(t => t.id)) + 1;
  ...
  const getFilteredTemplates = () => {
    ...
    Object.keys(templates).forEach(category => {
      if (filterCategory === 'all' || filterCategory === category) {
        filteredResults[category] = templates[category].filter(template => 
  ...
  return (
    <div className="w-full min-h-screen ">
      ...
      <div className="p-4">
        ...
        <p className="text-sm">Manage and configure required PPE for site tasks</p>
      </div>
      ...
      <div className=" border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex">
            {/* <button 
              className={`px-4 py-3 text-sm font-medium border-b-2 ${activeTab === 'templates' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              onClick={() => setActiveTab('templates')}
            >
              PPE Templates
            </button> */}
            {/* <button 
              className={`px-4 py-3 text-sm font-medium border-b-2 ${activeTab === 'induction' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              onClick={() => setActiveTab('induction')}
            >
              Induction Links
            </button>
            <button 
              className={`px-4 py-3 text-sm font-medium border-b-2 ${activeTab === 'siteEntry' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              onClick={() => setActiveTab('siteEntry')}
            >
              Site Entry Configuration
            </button>
            <button 
              className={`px-4 py-3 text-sm font-medium border-b-2 ${activeTab === 'reports' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
      ...
      <div className="max-w-7xl mx-auto p-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
             ...
             bg-white text-black dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 "
              ...
              <div className="flex gap-2 w-full md:w-auto">
                ...
                <button
                  className="btn btn-primary px-4 py-2 rounded-lg flex items-center gap-1"
              ...
              filteredTemplates[category].length > 0 && (
                <div key={category} className="mb-8">
                  ...
                  >
                    ...
                    <span className="text-gray-800">{expandedCategory === category ? '−' : '+'}</span>
                  </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-b-lg shadow-sm overflow-hidden">
                      {filteredTemplates[category].map(template => (
                        <div key={template.id} className="border-b last:border-b-0">
                          {currentEditing && currentEditing.id === template.id ? (
                            <div className="p-4">
                              <div className="mb-4">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setCurrentEditing({...currentEditing, taskName: e.target.value})}
                                />
                              </div>
                              ...
                              <div className="mb-4">
                                ...
                                <textarea
                                  ...
                                  onChange={(e) => setCurrentEditing({...currentEditing, description: e.target.value})}
                                />
                              </div>
                              
                              <div className="mb-4">
                                <div className="flex justify-between items-center mb-2">
                                  <label className="block text-sm font-medium text-gray-700">Required PPE</label>
                                  <button 
                                  ...
                                  <div className="bg-gray-50 p-3 rounded mb-3">
                                    <div className="flex flex-wrap gap-2 mb-3">
                                      <select
                                        ...
                                        onChange={(e) => setNewPPEItem({...newPPEItem, name: e.target.value})}
                                      >
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => setNewPPEItem({...newPPEItem, standard: e.target.value})}
                                      />
                                      ...
                                      <div className="flex items-center">
                                        <input
                                          ...
                                          onChange={(e) => setNewPPEItem({...newPPEItem, mandatory: e.target.checked})}
                                        />
                                        <label htmlFor="mandatoryCheck" className="ml-2 text-sm text-gray-700">Mandatory</label>
                                      </div>
                                    </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                  <thead className="bg-gray-50">
                                    <tr>
                                      ...
                                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mandatory</th>
                                      ...
                                      <tr key={index}>
                                        ...
                                        <td className="px-4 py-2 whitespace-nowrap">
                                          <input
                                            ...
                                            onChange={(e) => {
                                              const updatedItems = [...currentEditing.ppeItems];
                                              updatedItems[index].mandatory = e.target.checked;
                                              setCurrentEditing({...currentEditing, ppeItems: updatedItems});
                                            }}
                                            className="h-4 w-4 text-blue-600 rounded"
                                          />
                                        </td>
                            ...
                            <div className="p-4">
                              <div className="flex justify-between">
                                ...
                                <button
                                  ...
                                  onClick={() => handleEdit(template.id, category)}
                                >
                              ...
                              <div className="mt-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {template.ppeItems.map((item, index) => (
                                    <div 
                                      key={index} 
                                      className={`px-3 py-1 rounded-full text-xs flex items-center ${item.mandatory ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}
                                    >
                                      {item.mandatory && <AlertTriangle className="h-3 w-3 mr-1" />}
                                      {item.name}
                                    </div>
                                  ))}
                                </div>
                              ...
                              <div className="mt-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {template.assignedTo.map((assignee, index) => (
        ...
        {activeTab === 'induction' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-medium mb-4">Induction Links Configuration</h2>
            <p className="text-gray-600 mb-6">Connect PPE templates to induction types to display appropriate requirements during worker onboarding.</p>
            
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Induction Type</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Linked PPE Template</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                <tr>
                  <td className="px-6 py-4 whitespace-nowrap">General Site Induction</td>
                  <td className="px-6 py-4 whitespace-nowrap">General Site Access</td>
                ...
                <tr>
                  <td className="px-6 py-4 whitespace-nowrap">Demolition Team Induction</td>
                ...
                <tr>
                  <td className="px-6 py-4 whitespace-nowrap">Painting Team Induction</td>
        ...
        {activeTab === 'siteEntry' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-medium mb-4">Site Entry Configuration</h2>
            <p className="text-gray-600 mb-6">Configure how PPE requirements are displayed and confirmed during site sign-in.</p>
            ...
            <div className="border rounded-lg p-4 mb-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  <input id="requireAcknowledge" type="checkbox" className="h-4 w-4 text-blue-600 rounded" checked />
                  <label htmlFor="requireAcknowledge" className="ml-2 text-sm text-gray-700">
        ...
        { isAddingTemplate && (
          <Modal show={isAddingTemplate} onHide={() => setIsAddingTemplate(false)} centered size="lg">
            ...
            <div className="p-4">
             ...
             bg-white text-black dark:bg-gray-800 dark:text-white dark:placeholder-gray-400"
                              ...
                              <div className="mb-4">
                                <div className="flex justify-between items-center mb-2">
                                  <label className="block text-sm font-medium text-gray-500">Required PPE</label>
                                  <button 
                                    className="text-blue-600 text-sm flex items-center"
                                    onClick={() => setIsAddingPPE(true)}
                                  >
             ...
             bg-white text-black dark:bg-gray-800 dark:text-white dark:placeholder-gray-400"
                              ...
                              <div className="mb-4">
  <label className="block text-sm font-medium mb-1">Assign PPE Checklist To</label>
  <select
...
{currentEditing?.assignmentType === "subcontractor" && (
  <div className="mb-4">
    ...
    <select
      ...
      value={currentEditing.assignedTo}
      onChange={(e) =>
        setCurrentEditing({ ...currentEditing, assignedTo: e.target.value })
      }
    >
      ...
      {areas.map((area) => (
        <option key={area} value={area}>
          {area}
                              ...
                              <div className="flex justify-end gap-2">
                                <button
                                  className=" px-4 py-2 text-gray-500 hover:text-gray-800 border rounded border-gray-300" 
                                  onClick={() => setCurrentEditing(null)}
                                >
                                  Cancel
                                </button>
